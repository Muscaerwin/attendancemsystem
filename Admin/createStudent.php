...
if (isset($_POST['addStudent'])) {
    ...
    $rfid = mysqli_real_escape_string($conn, $rfid); // Sanitize RFID input
    ...
    $query = mysqli_query($conn, "SELECT * FROM tblstudents WHERE registrationNumber = '$registrationNumber' OR rfid = '$rfid'");
    $ret = mysqli_fetch_array($query);

    if ($ret > 0) { 
        $message = "Student with the given Registration No: $registrationNumber or RFID: $rfid exists!";
    } else {
        $query = mysqli_query($conn, "INSERT INTO tblstudents (firstName, lastName, email, registrationNumber, rfid, faculty, courseCode, studentImage1, studentImage2, dateRegistered) 
        VALUES ('$firstName', '$lastName', '$email', '$registrationNumber', '$rfid', '$faculty', '$courseCode', '$registrationNumber" . "_image1.png', '$registrationNumber" . "_image2.png', '$dateRegistered')");
...
if (isset($_POST['update'])) {
    ...
    $email = $_POST['email'];
    $registrationNumber = $_POST['registrationNumber'];
    $rfid = $_POST['rfid']; // Capture RFID for update
    $classId = $_POST['classId'];
    $classArmId = $_POST['classArmId'];

    $query = mysqli_query($conn, "UPDATE tblstudents SET firstName='$firstName', lastName='$lastName', email='$email', registrationNumber='$registrationNumber', rfid='$rfid', classId='$classId', classArmId='$classArmId' WHERE Id='$Id'");
                ...
                <form method="post">
                    ...
                    <div>
                        <div>
                            ...
                            <input type="text" required name="rfid" placeholder="RFID"> <!-- RFID input -->
